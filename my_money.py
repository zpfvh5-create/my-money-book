...
def save_data(df):
    try:
        ...
        # 시트에 쓰기 작업
        conn.update(data=df)
        st.cache_data.clear() # 캐시 즉시 비우기
        return True
    except Exception as e:
        st.error(f"⚠️ 저장 오류 발생: {e}") # 구체적인 에러 메시지 표시
        return False
...
def manage_entry(date_str):
    ...
    with st.container(border=True):
        ...
        if st.button("내역 저장", use_container_width=True, type="primary"):
            if uid and c:
...
if st.session_state.user_id is None:
    ...
    with col2:
        if st.session_state.login_page == 'login':
            ...
            st.title("📝 회원가입")
            nid = st.text_input("아이디 만들기")
            npw = st.text_input("비밀번호 만들기", type="password")
    ...
    state = calendar(events=events, options={"initialView": "dayGridMonth", "locale": "ko"}, key="v18")
